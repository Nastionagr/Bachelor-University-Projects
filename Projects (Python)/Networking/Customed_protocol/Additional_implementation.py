...
def sendData(serverIP, serverPort, clientSock, information, data, typ, lock):
    global SEND_ERROR, SENT_PACKETS
    error = SEND_ERROR
...
def startSending(serverIP, serverPort, KASock, clientSock, data, fragmentSize, file = False):
    ...
    print("Total size of the sent headers during whole communication is ", SENT_PACKETS, "* 10B(size of my header) = ", SENT_PACKETS*10, "B.")
    if typ == FILE:
...
def client():
    global C_CONNECTED, SEND_ERROR, SENT_PACKETS
